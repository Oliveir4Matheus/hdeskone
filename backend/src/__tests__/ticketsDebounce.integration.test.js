...
function buildApp(role = "admin") {
  // Re-require fresh instances after mocks are set
  ...
  // Inject user via middleware (simulates JWT auth)
...
describe("Funcionalidade 6: PUT /tickets/:id + debouncedNotify", () => {
  ...
  test("teste25: sem mudança real de valor não chama debouncedNotify", async () => {
    ...
    // Same status and priority
    prisma.ticket.findUnique.mockResolvedValue({ ...baseTicket, assignedTo: null });
    prisma.ticket.update.mockResolvedValue({ ...baseTicket, assignedTo: null });

    await request(app)
      .put("/tickets/1")
      .send({ status: "open", priority: "medium" }); // no real change
...
describe("Funcionalidade 7: POST /tickets/:id/messages + debouncedNotify", () => {
  ...
  const createdMsg = {
    id: 10,
    content: "Olá, temos uma atualização",
    userId: 99,
    ticketId: 1,
    user: { id: 99, name: "Ana Staff" },
  };

  test("teste27: mensagem de admin chama debouncedNotify com changeField Nova mensagem no chat", async () => {
    app = buildApp("admin");
    prisma = require("../lib/prisma");
    debouncedNotify = require("../lib/mailDebounce").debouncedNotify;
    ...
    await request(app)
      .post("/tickets/1/messages")
      .send({ content: "Olá, temos uma atualização" });

    expect(debouncedNotify).toHaveBeenCalledWith(
      expect.objectContaining({ id: 1 }),
      expect.objectContaining({
        changeField: "Nova mensagem no chat",
        chatMessage: "Olá, temos uma atualização",
      })
    );
  });
  ...
  test("teste29: mensagem de colaborador NÃO chama debouncedNotify", async () => {
    ...
    // Ticket belongs to the colaborador (same email)
    prisma.ticket.findUnique.mockResolvedValue({
      ...msgTicket,
      requesterEmail: "ana@test.com", // matches req.user.email
    });
    prisma.message.create.mockResolvedValue(createdMsg);

    await request(app)
      .post("/tickets/1/messages")
      .send({ content: "Pergunta do cliente" });
